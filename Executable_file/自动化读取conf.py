...
class Colony_Frame_Auto():
    ...
    def colony_frame_fb(self, fb=None):
        ...
        if fb is None:
            self.config.read(f'{fb}')
        else:
            if os.path.exists(f'{fb}') is True:
                self.config.read(f'{fb}')
            else:
    ...
    def colony_frame_info(self, fb=None):
        ...
        if fb is None:
            self.config.write(open('ECP.conf', 'w+'))
        else:
...
if __name__ == '__main__':
    ...
    #     colony_frame_fb()方法 用来读取指定文件
    colony_frame_auto.colony_frame_fb(fb='ECP.conf')
